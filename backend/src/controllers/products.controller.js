             ...
             if(res.accesstype == "admin-user"){
                   productModel.createProduct(productObj , (err , data)=>{
                        if(err){
                            ...
                            ress.json({status:201 ,message:'New product created successfully',productData:data})    
                        }  
                    })
             }else{
             ...
             if(res.accesstype == "admin-user"){
                   productModel.updateProduct( req.params.id,productObj , (err , data)=>{
                        if(err){
                            ...
                            ress.json({status:201 ,message:' Product updated successfully',productData:data})    
                        }  
                    })
             }else{
             ...
             if(res.accesstype == "admin-user"){
                   productModel.deleteProduct( req.params.id , (err , data)=>{
                        if(err){
                            ...
                            log(chalk.blue("Product Is Deleted Successfully ",JSON.stringify(data)));
